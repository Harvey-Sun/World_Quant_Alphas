...
Strategy_Name = 'WQ_alpha#041_10_100'
...
buy_number = 100
period = 10
s_or_b = True  # true表示选因子较小的股票，false表示选因子较大的股票
...
def init_per_day(sdk):
    ...
    if step == period:
        ...
        adj_f = pd.Series(sdk.getFieldData('LZ_GPA_CMFTR_CUM_FACTOR')[-1], index=stock_list)
        ...
        # 计算昨天的factor
        factor = ((adj_high * adj_low) ** 0.5 - (value / adj_volume) * 10) / adj_close  # 计算昨天的alpha41
        factor = np.abs(factor - factor.mean())
        # 选出排序靠前的股票
        factor[stop.notnull().any()] = np.nan  # 剔除今天停牌的股票
        factor.sort_values(ascending=s_or_b, inplace=True)
        stock_pool = factor.index[:buy_number]  # 选出alpha41因子较小的股票
