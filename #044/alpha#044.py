...
Strategy_Name = 'WQ_alpha#044_10_100'
...
buy_number = 100
period = 10
lowest = False  # 是否选因子值最小的股票，False为选取因子最大的股票
...
def init_per_day(sdk):
    ...
    if step == period:
        ...
        factor = {}
        for i in adj_high.columns:
            factor[i] = pd.concat([adj_high[i], volume_rank[i]], axis=1).corr().iloc[0,1]
        factor = pd.Series(factor)
        factor[stop.notnull().any()] = np.nan  # 剔除停牌的股票
        factor.sort_values(ascending=lowest, inplace=True)
        # 选取股票
